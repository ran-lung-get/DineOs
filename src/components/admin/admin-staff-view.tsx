...
export function AdminStaffView({
  ...
  const filteredUsers = safeUsers.filter((u) => {
    ...
    const name = String(u.display_name || u.full_name || u.fullName || "").toLowerCase();
          ...
          <tbody className="divide-y divide-slate-100 font-semibold text-slate-700">
            {filteredUsers.length === 0 ? (
              ...
              filteredUsers.map((user) => {
                ...
                const pictureUrl = user.picture_url || user.avatar_url || null;
                const initials = displayName ? displayName.slice(0, 2).toUpperCase() : "US";
                const userRole = String(user.role || "customer").toLowerCase();

                return (
                  <tr key={user.id || user._id || displayName} className="hover:bg-slate-50/50">
                    <td className="py-3 px-4 flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-extrabold text-[#002e47]">{displayName}</p>
                    ...
                    <td className="py-3 px-4">
                      {userRole === "captain" ? (
                        ...
                        <button
                          onClick={() => toggleUserActiveStatus(user.id || user._id, isActive)}
                    ...
                    <td className="py-3 px-4 text-right space-x-1.5">
                      {userRole !== "captain" ? (
                        <div className="inline-flex gap-1.5 justify-end items-center">
                          <button
                            onClick={() => updateUserRole(user.id || user._id, "admin")}
                          ...
                          <button
                            onClick={() => updateUserRole(user.id || user._id, "staff")}
                            className={`px-2 py-1 rounded text-[10px] font-bold border transition cursor-pointer ${
                              userRole === "staff"
                          ...
                          <button
                            onClick={() => updateUserRole(user.id || user._id, "customer")}
                            className={`px-2 py-1 rounded text-[10px] font-bold border transition cursor-pointer ${
                              userRole === "customer"
                          ...
                          <button
                            onClick={() => deleteUser(user.id || user._id, displayName)}
