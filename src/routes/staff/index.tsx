...
function KitchenSidebarContent({
  ...
  view: "kitchen" | "tables" | "menu";
  setView: (v: "kitchen" | "tables" | "menu") => void;
  onClose?: () => void;
}) {
  return (
      ...
      <AnimatePresence>
        {sidebarOpen && (
          <>
            ...
            >
              <KitchenSidebarContent view={view} setView={setView} onClose={() => setSidebarOpen(false)} />
            </motion.aside>
          </>
        )}
      </AnimatePresence>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-sm font-black tracking-tight" style={{ color: BRAND }}>
                  {view === "kitchen" ? "ครัวลุงเกตุ" : view === "tables" ? "ผังโต๊ะ" : "จัดการเมนู"}
                </h1>
                <p className="text-[9px] font-bold text-slate-500">
                  {view === "kitchen" ? `คิวค้าง: ${stats.totalActive}` : view === "tables" ? "จัดการผังโต๊ะ" : "เมนูอาหาร"}
                </p>
              </div>
            </div>
          </div>
